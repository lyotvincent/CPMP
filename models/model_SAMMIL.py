...
from models.sammil_modules.swin import *
...
class sam_mil(nn.Module):
    ...
    @torch.no_grad()
    def forward_test(self, x, return_attn=False, no_norm=False, **kwargs):
        x = x.unsqueeze(0)
        x = self.patch_to_emb(x)
        x = self.dp(x)
        ...
        return logits, Y_prob, Y_hat, x, results_dict
    ...
    # for train loop
    def forward_train(self, x, attn=None, teacher_cls_feat=None, i=None, label=None, criterion=None, pred_correct=False,
                is_group_feat=None, relative_area=None, **kwargs):
        x = x.unsqueeze(0)
        x = self.patch_to_emb(x)
        x = self.dp(x)
        ...
        _, attn, act = self.online_encoder(x, return_attn=True, return_act=True)
        self.score_label = None if self.score_label == -1 else self.score_label
        attn = get_cam_1d_trans(self.predictor, act, attn[0], self.score_label,
                                self.online_encoder.attn_model.layer2.attn.to_out)
        ...
        # cl loss
        Y_prob = torch.softmax(student_logit, dim=1)
        Y_hat = torch.argmax(Y_prob, dim=1).unsqueeze(1)  # 1 x num_cls

        results_dict = {"embedding": student_cls_feat}

        return student_logit, Y_prob, Y_hat, len_keep, results_dict


    def forward(self, x, attn=None, teacher_cls_feat=None, idx=None, label=None, criterion=None, pred_correct=False,
                is_group_feat=None, relative_area=None, numGroup=3, is_train=False, **kwargs):
        if not is_train:
            return self.forward_test(x)

        total_logits, Y_prob, Y_hat, keep_num, results_dict = self.forward_train(x, None, None, i=idx,
        ...
        for tindex in index_chunk_list:
            subFeat_tensor = torch.index_select(x, dim=0, index=tindex.to(device))

            tPredict, _, _, _, _ = self.forward_train(subFeat_tensor, None, None, i=idx)
        ...
        return total_logits, Y_prob, Y_hat, slide_sub_preds, results_dict
