...
class Attention(nn.Module):
    def __init__(self, dim=512, heads=8, dim_head=512 // 8, dropout=0.1, **kwargs):
        super().__init__()
    ...
    def forward(self, x, register_hook=False):
        ...
        return self.to_out(out), self.attention_map
...
class AgentAttention(nn.Module):
    def __init__(self, dim=512, heads=8, dim_head=512 // 8, dropout=0.1, agent_num=10):
        super().__init__()
        inner_dim = dim_head * heads
        project_out = not (heads == 1 and dim_head == dim)
        ...
        self.agent_token = nn.Parameter(torch.randn(1, self.heads, agent_num, dim_head))
    ...
    def forward(self, x, register_hook=False):
        ...
        agent_attn = torch.matmul(self.agent_token, k.transpose(-1, -2)) * self.scale # [b, head, agent_num, n]
        agent_v = torch.matmul(self.attend(agent_attn), v) # [b, head, agent_num, dim]

        q_attn = torch.matmul(q, self.agent_token.transpose(-1, -2)) * self.scale # [b, head, n, agent_num]
        out = torch.matmul(self.attend(q_attn), agent_v) # [b, head, n, dim]

        out = rearrange(out, 'b h n d -> b n (h d)')
        
        out[:, 1:, :] = out[:, 1:, :] + rearrange(v, 'b h n d -> b n (h d)')[:, 1:, :]

        if register_hook:
            try: # we first try to do it on GPU
                attn_matrix = torch.matmul(self.attend(q_attn), self.attend(agent_attn)) # [b, head, n, n]
                # attn_matrix = self.attend(torch.matmul(q_attn, agent_attn)) # [b, head, n, n]
            except: # if it fails, we do it on CPU
                attn_matrix = torch.matmul(self.attend(q_attn).to("cpu"), self.attend(agent_attn).to("cpu")) # [b, head, n, n]
        else:
            attn_matrix = None
        
        return self.to_out(out), attn_matrix
...
def rollout(attentions, discard_ratio, head_fusion):
    ...
    for attention in attentions: # attention shape: [1, num_heads, num_patches+1, num_patches+1]
        if head_fusion == "mean":
            attention_heads_fused = attention.mean(axis=1)
        elif head_fusion == "max":
            attention_heads_fused = attention.max(axis=1)[0] # [1, num_patches+1, num_patches+1]
        ...
        _, indices = flat.topk(int(flat.size(-1)*discard_ratio), -1, False) # 90% is discard.
