...
The difference between generate_split_CrossValidation() and the above code is that,
in the above code, each split's validation set overlaps; 
whereas in standard cross-validation, the data is first divided into n folds, with each fold not overlapping, 
and then each fold is used as the validation set. The implementation below achieves this.
"""
