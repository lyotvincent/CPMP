...
class WsiPatching(object):
    ...
    def set_params(preset):
        """
        The kernel for the close operation is 4x4. Use Otsu's thresholding method (True); set the maximum number of holes to 10.
        If Otsu's method is used (True), 'sthresh' is not needed. 'sthresh' is only used for simple binary thresholding.
        Modify patch parameters: set 'use_padding' to False.
        Add visualization parameters: 'draw_grid': True.
        Convert the image to HSV, then apply median filtering to the saturation space of hue (H), saturation (S), and value (V), and perform Otsu's thresholding.

        sthresh: Threshold value for thresholding; this parameter is invalid when 'use_otsu' is True.
        mthresh: Threshold value for median filtering.
        close: Kernel parameter for the close operation after thresholding.

        a_t: Foreground area threshold after removing all holes in the foreground region; in actual calculation, filter_params['a_t'] * 512 / level_downsamples[seg_level].
        a_h: Hole area threshold; holes smaller than this value are not considered.
        max_n_holes: Filter holes; keep at most max_n_holes holes within each foreground contour.
        """
    ...
    def seg_and_patch(self, sourcepath, save_dir, patch_save_dir, mask_save_dir, stitch_save_dir, 
        ...
        for i in range(total):
            ...
            if bgtissue:
                ...
                mask_path = os.path.join(mask_save_dir, slide_id+'.png') # Save the image as png format, not jpg
                mask.save(mask_path)
    ...
    def update_dfinfo(df, idx, WSI_object, vis_level, seg_level):
        ...
        """
        # Update the slide information columns in the dataframe
